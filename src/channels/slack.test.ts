...
describe('SlackChannel', () => {
  ...
  describe('sendMessageAs', () => {
    it('sends message with username and icon_emoji overrides', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();

      await channel.sendMessageAs(
        'slack:C0123456789',
        'Research complete',
        'Researcher',
      );
      ...
      expect(currentApp().client.chat.postMessage).toHaveBeenCalledWith({
        ...
        username: 'Researcher',
        icon_emoji: expect.stringMatching(/^:\w+:$/),
      });
    });

    it('assigns deterministic icon per sender name', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();

      await channel.sendMessageAs('slack:C0123456789', 'First', 'Researcher');
      await channel.sendMessageAs('slack:C0123456789', 'Second', 'Researcher');

      const calls = currentApp().client.chat.postMessage.mock.calls;
      expect(calls[0][0].icon_emoji).toBe(calls[1][0].icon_emoji);
    });

    it('assigns different icons for different sender names', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();

      await channel.sendMessageAs('slack:C0123456789', 'A', 'Researcher');
      await channel.sendMessageAs('slack:C0123456789', 'B', 'Writer');

      const calls = currentApp().client.chat.postMessage.mock.calls;
      // Different names should get different icons (statistically — hash collision possible but unlikely for these names)
      expect(calls[0][0].username).toBe('Researcher');
      expect(calls[1][0].username).toBe('Writer');
    });

    it('queues sendMessageAs when disconnected', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);

      await channel.sendMessageAs('slack:C0123456789', 'Queued', 'Researcher');

      expect(currentApp().client.chat.postMessage).not.toHaveBeenCalled();
    });

    it('flushes sendMessageAs messages with sender on connect', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
    ...
    it('splits long sendMessageAs messages', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();

      const longText = 'X'.repeat(4500);
      await channel.sendMessageAs('slack:C0123456789', longText, 'Researcher');
    ...
    it('queues sendMessageAs on send failure', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
      ...
      await expect(
        channel.sendMessageAs('slack:C0123456789', 'Will fail', 'Researcher'),
      ).resolves.toBeUndefined();
    });
  });
