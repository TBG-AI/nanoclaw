...
- `perf` — added when the finding came from the latency scan (Phase 1b)

### Latency issue variant (from high-latency scan)

When the finding came from BugReporter's latency scan phase rather than the 5xx error scan, the issue body uses a **latency-specific frontmatter block** instead of the error block. Both scans may produce issues on the same endpoint — those get merged (§A dedup rules, extended: *"same endpoint + same flow + within 1h window → merge, union the fields, max severity"*).

```markdown
<!-- tbg-bug-report-latency -->
**Signal type**: `latency`
**Service**: `backend-server` | `backend-odds`
**Flow**: `{flow_name}` · class: `user-interactive|write|read-heavy|internal`
**Endpoint**: `{METHOD} {path}`
**Window**: `{window_minutes}`m ending `{iso8601}`
**Sample count**: `{int}` requests in window
**Latency percentiles**:
- p50: `{ms}`ms
- p95: `{ms}`ms (SLO: `{target_ms}`ms — **{ratio}x SLO**)
- p99: `{ms}`ms (SLO: `{target_ms}`ms — **{ratio}x SLO**)
**Severity**: `CRITICAL | HIGH | MEDIUM | LOW`
**Duration above SLO**: `{minutes}`m (sustained? yes/no)
```

Required sections for latency issues:
1. **Sample slow trace IDs** — up to 3 from Tempo, with durations, so BugFixer can reproduce via `/debug-logs trace <id>`
2. **Latency distribution** — bucket counts from the histogram, showing where the slow tail is
3. **Recent deploys** — list of deploys in the window that might have introduced the regression (link to deploy log)
4. **Correlated errors** — if the error scan also flagged this endpoint, link that finding
5. **Grafana links**:
   - Prometheus p95/p99 panel
   - Tempo slow-trace search
   - If a flow diagram exists: link to the relevant PlantUML in `specs/flows/`
6. **Suggested owner** — usually `/f2-swe` or `/f3-swe` for profiling, not BugFixer (BugFixer is for code bugs, not perf)

**Why latency issues have a different flow**: the fix for a perf issue is profiling + measurement + targeted optimization, not "write a failing test and fix". BugFixer's test-first workflow doesn't apply — you can't write a test that "fails because it's slow" without adding flakiness. Latency issues should go to `/f2-swe` / `/f3-swe` with a `perf` label and explicit "not for BugFixer" note.
